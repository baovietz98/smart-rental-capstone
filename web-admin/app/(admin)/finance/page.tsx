...
export default function FinancePage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const params: any = { take: 50 }; // Fetch up to 50 for the selected month
      if (selectedMonth) {
        params.month = selectedMonth.format("MM-YYYY");
      }
      ...
      setStats(statsRes.data || { income: 0, expense: 0, net: 0 });
    } catch (error) {
  ...
  const handleExportExcel = () => {
    ...
    const csvContent = [
      ...
      ...transactions.map((tx) => {
        ...
        const sign = tx.type === "EXPENSE" || tx.type === "OTHER" ? "-" : "+";
        const roomStr = tx.contract?.room?.name
          ? `Phòng ${tx.contract.room.name}`
          : "";
        const tenantStr = tx.contract?.tenant?.name
    ...
    const monthStr = selectedMonth ? selectedMonth.format("MM_YYYY") : "Tat_Ca";

    link.setAttribute("href", url);
    link.setAttribute("download", `Bao_Cao_Tai_Chinh_${monthStr}.csv`);
        ...
        <header className="flex justify-between items-end mb-12">
          ...
          <div className="flex gap-3">
            ...
            <DatePicker
              picker="month"
              value={selectedMonth}
              onChange={(date) => setSelectedMonth(date as Dayjs)}
              format="MM/YYYY"
              allowClear={false}
              className="h-[40px] border-none bg-[#F4F4F0] hover:bg-[#EBEBE6] focus:bg-[#EBEBE6] shadow-sm rounded-lg claude-btn-secondary px-4 cursor-pointer"
            />
            <button
